const GBK_WALLET_FIX_VERSION="2026-09-14-wallet-v4";
...
async function transformGBKIndex(response){
 try{
  ...
  let out=html;
  out=out.replace('<script type="module">','<script src="https://cdn.jsdelivr.net/npm/ethers@6.15.0/dist/ethers.umd.min.js"></script>\\n<script type="module">');
  out=out.replace(/import \\* as ethers from "https:\\/\\/esm\\.sh\\/ethers@6\\.13\\.5";\\s*import \\{\\s*createAppKit\\s*\\} from "https:\\/\\/esm\\.sh\\/@reown\\/appkit\\?bundle";\\s*import \\{\\s*EthersAdapter\\s*\\} from "https:\\/\\/esm\\.sh\\/@reown\\/appkit-adapter-ethers\\?bundle";\\s*import \\{\\s*bsc\\s*\\} from "https:\\/\\/esm\\.sh\\/@reown\\/appkit\\/networks\\?bundle";/,'const ethers = window.ethers;\\nif(!ethers) throw new Error("Ethers library failed to load");');
  ...
  if(blockStart>=0 && blockEnd>blockStart){
   const stable=[
    ...
    " walletProvider=provider;ethersProvider=new ethers.BrowserProvider(provider);",
    ...
    " signer=await ethersProvider.getSigner();walletAddress=await signer.getAddress();return true;",
    "}",
    "",
    "/* =====================================================",
    "   INJECTED WALLET"
   ].join("\\n");
   out=out.slice(0,blockStart)+stable+out.slice(blockEnd+"/* =====================================================\\n   INJECTED WALLET".length);
  }
  out=out.replace("let appKit=null;","let appKit=null;\\nlet wcProvider=null;");
  const headers=new Headers(response.headers);
  headers.set("Cache-Control","no-store");
  headers.set("X-GBK-Wallet-Fix",GBK_WALLET_FIX_VERSION);
  return new Response(out,{status:response.status,statusText:response.statusText,headers});
 }catch(e){console.error("GBK wallet transform failed",e);return response;}
}
...
self.addEventListener("fetch",(event)=>{
 ...
 if(url.origin===self.location.origin && (url.pathname==="/" || url.pathname==="/index.html")){
  event.respondWith(fetch(event.request).then(transformGBKIndex).catch(()=>caches.match(event.request)));
 }else{
